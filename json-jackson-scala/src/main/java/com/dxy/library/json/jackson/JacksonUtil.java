...
public class JacksonUtil {
     ...
     * @param type 带范型的类型
     */
    public static <V> V from(String json, ParameterizedType type) {
        if (StringUtils.isEmpty(json)) {
            return null;
        }
        try {
            ...
            List<JavaType> javaTypes = Arrays.stream(actualTypeArguments)
                .map(actualType -> mapper.getTypeFactory().constructType(actualType))
                .collect(Collectors.toList());
            JavaType[] javaTypesArray = javaTypes.toArray(new JavaType[actualTypeArguments.length]);
            JavaType javaType = mapper.getTypeFactory().constructParametricType((Class<?>)rawType, javaTypesArray);
            return mapper.readValue(json, javaType);
        } catch (IOException e) {
            throw new JacksonException("jackson from error, json: {}, type: {}", json, type, e);
        }
     ...
     * @return V<T>
     */
    public static <V> V from(String json, Class<V> type, Class<?> rawType) {
        try {
            JavaType javaType = mapper.getTypeFactory().constructParametricType(type, rawType);
            return mapper.readValue(json, javaType);
        } catch (IOException e) {
            throw new JacksonException("jackson from error, json: {}, type: {}, rawType: {}", json, type, rawType, e);
        }
    }
     ...
     * JSON反序列化
     */
    public static <V> V from(byte[] json, Class<V> type) {
        return from(json, (Type)type);
    }
     ...
     * JSON反序列化
     */
    public static <V> V from(byte[] json, TypeReference<V> type) {
        return from(json, type.getType());
    }
     ...
     * JSON反序列化
     */
    public static <V> V from(byte[] json, Type type) {
